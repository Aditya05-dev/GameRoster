...
import { snapshotSchema } from "../services/buildSnapshot.js";
...
buildsRouter.post("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const character = await pool.query(
      ...
      [b.characterId, b.gameId],
    );
    if (!character.rowCount)
      return res
        .status(422)
        .json({ error: "Character does not belong to this game." });
    if (JSON.stringify(b.snapshot || {}).length > 50000)
      return res.status(422).json({ error: "Build snapshot is too large." });
    if (b.snapshot) delete b.snapshot.uid;
    const result = await pool.query(
      `INSERT INTO builds (user_id, game_id, character_id, title, build_type, equipment_ids, main_stats,
                            substat_priority, skill_priority, team_recommendation, notes, visibility, snapshot)
...
buildsRouter.patch("/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    if (existing.rows[0].user_id !== req.user.id && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "You can only edit your own builds." });
    }
    const b = buildSchema.partial().parse(req.body);
    if (b.snapshot) {
      if (JSON.stringify(b.snapshot).length > 50000)
        return res.status(422).json({ error: "Build snapshot is too large." });
      delete b.snapshot.uid;
    }
    const fields = [];
    const values = [];
    let i = 1;
    for (const [key, col] of [
      ...
      ["substatPriority", "substat_priority"],
    ...
    const result = await pool.query(
      `UPDATE builds SET ${fields.join(", ")}, updated_at = now() WHERE id = $${i} RETURNING *`,
      values,
    );
...
buildsRouter.delete("/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    if (existing.rows[0].user_id !== req.user.id && req.user.role !== "admin") {
      return res
        .status(403)
        .json({ error: "You can only delete your own builds." });
    }
