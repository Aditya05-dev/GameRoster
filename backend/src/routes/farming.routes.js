...
const planSchema = z.object({
  ...
  materialsNeeded: z.record(z.number().int().min(0).max(100000000)).optional(),
  materialsCollected: z
    .record(z.number().int().min(0).max(100000000))
    .optional(),
});
...
farmingRouter.get("/", requireAuth, async (req, res, next) => {
  try {
    const result = await pool.query(
      `SELECT * FROM farming_plans WHERE user_id = $1 ORDER BY created_at DESC`,
      [req.user.id],
    );
...
farmingRouter.post("/", requireAuth, async (req, res, next) => {
  try {
    ...
    const character = await pool.query(
      "SELECT 1 FROM characters WHERE id=$1 AND game_id=$2 AND is_visible AND archived_at IS NULL",
      [p.characterId, p.gameId],
    );
    if (!character.rowCount)
      return res
        .status(422)
        .json({ error: "Character does not belong to this game." });
    const result = await pool.query(
...
farmingRouter.patch("/:id", requireAuth, async (req, res, next) => {
  try {
    const existing = await pool.query(
      `SELECT user_id FROM farming_plans WHERE id = $1`,
      [req.params.id],
    );
    ...
    if (existing.rows[0].user_id !== req.user.id)
      return res
        .status(403)
        .json({ error: "You can only edit your own plans." });
    const p = planSchema.partial().parse(req.body);
    const fields = [];
    const values = [];
    let i = 1;
    for (const [key, col] of [
      ["targetUpgrades", "target_upgrades"],
      ["materialsNeeded", "materials_needed"],
      ["materialsCollected", "materials_collected"],
    ]) {
      if (p[key] !== undefined) {
        fields.push(`${col} = $${i++}`);
        values.push(p[key]);
    ...
    const result = await pool.query(
      `UPDATE farming_plans SET ${fields.join(", ")}, updated_at = now() WHERE id = $${i} RETURNING *`,
      values,
    );
...
farmingRouter.delete("/:id", requireAuth, async (req, res, next) => {
  try {
    ...
    if (existing.rows[0].user_id !== req.user.id)
      return res
        .status(403)
        .json({ error: "You can only delete your own plans." });
    await pool.query(`DELETE FROM farming_plans WHERE id = $1`, [
      req.params.id,
    ]);
    res.status(204).end();
  } catch (err) {
